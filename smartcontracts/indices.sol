...
/**
 * @title IndicesConsumer
 * @notice Retrieves indices data from Token Metrics API via Chainlink Functions
 */
contract IndicesConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            'const INDICES_API_URL = "https://api.tokenmetrics.com/v2/indices";',
            ...
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No indices data received');",
            "const item = apiResponse.data[0];",
            'if (!item || !item.TICKER) throw Error("Invalid indices data");',
            "return Functions.encodeString(item.TICKER);"
        );
    ...
    function fulfillRequest(
        ...
        latestTicker = abi.decode(response, (string));

        emit Response(requestId, latestTicker, response, err);
    }

    /**
     * @notice Get the latest ticker
     */
