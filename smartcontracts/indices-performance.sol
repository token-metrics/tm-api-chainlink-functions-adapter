...
/**
 * @title IndicesPerformanceConsumer
 * @notice Retrieves indices performance data from Token Metrics API via Chainlink Functions
 */
contract IndicesPerformanceConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            'if (!apiResponse || !apiResponse.success) throw Error(`Indices Performance API error: ${apiResponse?.message || "Unknown error"}`);',
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0 || !apiResponse.data[0].INDEX_CUMULATIVE_ROI) throw Error('No indices performance data received');",
    ...
    function fulfillRequest(
        ...
        cumulativeROI = abi.decode(response, (uint256));

        emit Response(requestId, cumulativeROI, response, err);
    }

    /**
     * @notice Get the latest cumulative ROI in wei (18 decimals)
     */
    function getLatestROI() external view returns (uint256) {
