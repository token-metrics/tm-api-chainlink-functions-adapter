...
/**
 * @title MarketMetricsConsumer
 * @notice Retrieves market metrics data from Token Metrics API via Chainlink Functions
 */
contract MarketMetricsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    uint256 public totalCryptoMarketCap;
    ...
    string internal source =
        string.concat(
            ...
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No market metrics data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.TOTAL_CRYPTO_MCAP !== "number") throw Error("Invalid market metrics data received");',
    ...
    function fulfillRequest(
        ...
        totalCryptoMarketCap = abi.decode(response, (uint256));

        emit Response(requestId, totalCryptoMarketCap, response, err);
    }

    /**
     * @notice Get the latest total crypto market cap in wei (18 decimals)
     */
