...
/**
 * @title HourlyOHLCVConsumer
 * @notice Retrieves hourly OHLCV data from Token Metrics API via Chainlink Functions
 */
contract HourlyOHLCVConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No hourly OHLCV data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.OPEN !== "number") throw Error("Invalid hourly OHLCV data received");',
