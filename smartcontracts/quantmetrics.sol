...
/**
 * @title QuantmetricsConsumer
 * @notice Retrieves token quantmetrics data from Token Metrics API via Chainlink Functions
 */
contract QuantmetricsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            'const QUANTMETRICS_API_URL = "https://api.tokenmetrics.com/v2/quantmetrics";',
            ...
            'if (!apiResponse || !apiResponse.success) throw Error(`Quantmetrics API error: ${apiResponse?.message || "Unknown error"}`);',
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No quantmetrics data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.VOLATILITY !== "number") throw Error("Invalid quantmetrics data received");',
            "const volatility = ethers.parseUnits(item.VOLATILITY.toString(), 18);",
            "return Functions.encodeUint256(volatility);"
        );
    ...
    function fulfillRequest(
        ...
        volatility = abi.decode(response, (uint256));

        emit Response(requestId, volatility, response, err);
    }

    /**
     * @notice Get the latest volatility in wei (18 decimals)
     */
