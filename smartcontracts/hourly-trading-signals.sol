...
/**
 * @title HourlyTradingSignalsConsumer
 * @notice Retrieves hourly trading signals data from Token Metrics API via Chainlink Functions
 */
contract HourlyTradingSignalsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No hourly trading signals data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.SIGNAL !== "string") throw Error("Invalid hourly trading signals data received");',
