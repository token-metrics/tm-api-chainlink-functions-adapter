...
/**
 * @title InvestorGradesConsumer
 * @notice Retrieves investor grades data from Token Metrics API via Chainlink Functions
 */
contract InvestorGradesConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            'if (!apiResponse || !apiResponse.success) throw Error(`Investor Grades API error: ${apiResponse?.message || "Unknown error"}`);',
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No investor grades data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.TM_INVESTOR_GRADE !== "number") throw Error("Invalid investor grades data received");',
    ...
    function fulfillRequest(
        ...
        investorGrade = abi.decode(response, (uint256));

        emit Response(requestId, investorGrade, response, err);
    }

    /**
     * @notice Get the latest investor grade in wei (18 decimals)
     */
    function getInvestorGrade() external view returns (uint256) {
