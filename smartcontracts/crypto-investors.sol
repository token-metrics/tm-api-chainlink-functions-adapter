...
/**
 * @title CryptoInvestorsConsumer
 * @notice Retrieves crypto investors data from Token Metrics API via Chainlink Functions
 */
contract CryptoInvestorsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            'if (!apiResponse || !apiResponse.success) throw Error(`Crypto Investors API error: ${apiResponse?.message || "Unknown error"}`);',
            "if (!apiResponse.data || !Array.isArray(apiResponse.data) || apiResponse.data.length === 0) throw Error('No crypto investors data received');",
            "const item = apiResponse.data[0];",
            'if (!item || typeof item.INVESTOR_NAME !== "string") throw Error("Invalid crypto investors data received");',
    ...
    function fulfillRequest(
        ...
        topInvestorName = abi.decode(response, (string));

        emit Response(requestId, topInvestorName, response, err);
    }

    /**
     * @notice Get the latest top investor name
     */
    function getTopInvestorName() external view returns (string memory) {
        return topInvestorName;
    }
} 