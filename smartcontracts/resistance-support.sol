...
/**
 * @title ResistanceSupportConsumer
 * @notice Retrieves resistance and support levels from Token Metrics API via Chainlink Functions
 */
contract ResistanceSupportConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string internal source =
        string.concat(
            ...
            " params: { token_id: tokenId, limit: 1, page: 1 },",
            ...
            'if (!apiResponse || !apiResponse.success) throw Error(`Resistance & Support API error: ${apiResponse?.message || "Unknown error"}`);',
            "if (!apiResponse.data || !Array.isArray(apiResponse.data[0].HISTORICAL_RESISTANCE_SUPPORT_LEVELS) || apiResponse.data[0].HISTORICAL_RESISTANCE_SUPPORT_LEVELS.length === 0) throw Error('No resistance and support levels data received');",
            "const topLevel = apiResponse.data[0].HISTORICAL_RESISTANCE_SUPPORT_LEVELS[0];",
            'if (!topLevel || typeof topLevel.level !== "number") throw Error("Invalid resistance and support level data received");',
    ...
    function fulfillRequest(
        ...
        topLevel = abi.decode(response, (uint256));

        emit Response(requestId, topLevel, response, err);
    }

    /**
     * @notice Get the latest top level in wei (18 decimals)
     */
    function getTopLevel() external view returns (uint256) {
        return topLevel;
    }
} 