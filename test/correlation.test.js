...
console.log("🧪 Token Metrics Correlation Adapter Test\n");

// Load the correlation adapter source code
const sourceCode = fs.readFileSync(
  "./functions/correlation.js",
  "utf8"
);

// Helper function to decode correlation data
function decodeCorrelationData(hexString) {
  try {
    if (!hexString) {
    ...
    throw new Error(
      `Failed to decode correlation data: ${error.message}`
    );
  }
}
...
async function testCorrelation() {
  ...
  try {
    const result = await simulateScript({
      ...
      args: ["36697"], // Using TMAI token ID as an example
    ...
    const topCorrelatedToken = decodeCorrelationData(result.responseBytesHexstring);

    console.log("✅ SUCCESS! Correlation data:");
    console.log(`   Token ID: 36697 (TMAI)`);
    console.log(`   Top Correlated Token: ${topCorrelatedToken}`);
    console.log(
